...
class BachelorDetails(models.AbstractModel):
    _name = 'report.ums.print_bachelor_details_english_edu'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        query = _("""select std.english_name , std.student_number , std.final_result , std.grade_date , 
        program.english_name ,nation.english_name, college.english_name,college.dean_english_name,
        college.registrar_english_name,spec.english_name from ums_student as std join ums_college as college ON college.id = 
        std.college_id join ums_program as program ON program.id = std.program join ums_nationality as nation ON 
        nation.id = std.nationality_id join ums_specialist as spec on spec.id = std.specialist_id and std.program IS NOT NULL and std.nationality_id IS NOT NULL and std.id = %s 
        """) % (
        ...
        if vals:
            for va in vals:
                ...
                student_number = va[1]
                final_result = va[2]
                grade_date = va[3].strftime("%d/%m/%Y")
                program = va[4]
                nationality_id = va[5]
                college = va[6]
                college_dean = va[7]
            ...
            if vals:
                for va in vals:
                    ...
                    query = _("""select note.note , note.note_en_details
                                                 ...
                                                 where note.note_id = %s and note.student_id = %s""") % (str(va[6]),str(student))
                    self.env.cr.execute(query)
                    notes = self.env.cr.fetchall()
                    for n in notes:
                        found = False
               ...
               # student obj to solve special problem shortly
                ...
                certificate_data.append({
                    ...
                    'degree_letter': student_obj.final_result_letter.english_name if student_obj else False,
