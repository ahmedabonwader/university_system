...
class DeplomaDetails(models.AbstractModel):
    _name = 'report.ums.print_deploma_details_arabic'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        query = _("""select std.name , std.student_number , std.final_result , std.grade_date ,
                    ...
                    and std.program IS NOT NULL and std.nationality_id IS NOT NULL and std.id = %s """) % (
        ...
        if vals:
            ...
            if vals:
                ...
                query = _("""select name, maximum_marks, minimum_marks
                                from ums_division where college_id = %s order by maximum_marks desc""") % (
                    str(student_obj.college_id.id))
                ...
                certificate_data.append({
                    ...
                    'program': program,
                    'specialist': student_obj.specialist_id.name if student_obj else False,
